...
if ([string]::IsNullOrWhiteSpace($github_api_url)) {
    ...
    Write-Host "Set it using: `$env:GITHUB_API_URL = 'your-github-api-url-here'" -ForegroundColor Yellow
    exit 1
}
...
while ($queue.Count -gt 0 -or $inProgress.Count -gt 0) {
    ...
    while ($inProgress.Count -lt $MaxConcurrent -and $queue.Count -gt 0) {
        ...
        $scriptBlock = {
            param($adoOrg, $adoTeamProject, $adoRepo, $githubOrg, $githubRepo, $gh_repo_visibility, $logFile, $workDir)
            ...
            function Migrate-Repository {
                ...
                (Get-Date), $adoTeamProject, $adoRepo, $githubOrg, $githubRepo, $gh_repo_visibility |
                Tee-Object -FilePath $logFile -Append | Out-Null

                "[{0}] [DEBUG] Running: gh ado2gh migrate-repo --ado-org {1} --ado-team-project {2} --ado-repo {3} --github-org {4} --github-repo {5} --target-api-url {6} --target-repo-visibility {7}" -f `
                (Get-Date), $adoOrg, $adoTeamProject, $adoRepo, $githubOrg, $githubRepo, $github_api_url, $gh_repo_visibility |
                Tee-Object -FilePath $logFile -Append | Out-Null
        ...
        $job = Start-Job -ScriptBlock $scriptBlock -ArgumentList $adoOrg, $adoTeamProject, $adoRepo, $githubOrg, $githubRepo, $gh_repo_visibility, $logFile, $workDir
