...
if [[ -z "$github_api_url" ]]; then
  echo -e "\033[31m[ERROR] GITHUB_API_URL env not set.\033[0m"
  echo -e "\033[33mExport it using: export GITHUB_API_URL=\"your-github-api-url-here\"\033[0m"
  exit 1
fi
...
migrate_repository() {
...

  {
    ...
    printf '[%s] [DEBUG] Running: gh ado2gh migrate-repo --ado-org %s --ado-team-project %s --ado-repo %s --github-org %s --github-repo %s --target-api-url %s --target-repo-visibility %s\n' \
      "$(date)" "${ado_org}" "${ado_team_project}" "${ado_repo}" "${github_org}" "${github_repo}" "${github_api_url}" "${gh_repo_visibility}"
